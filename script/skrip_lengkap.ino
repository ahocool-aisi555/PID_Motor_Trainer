...
volatile float TARGET_RPM = 400.0;        // Setpoint RPM (TARGET)
...
<head>
    ...
    <!-- Chart.js for real-time line chart -->
    ...
    <style>
        ...
        .status-box, .control-box, .chart-box {
            margin-top: 20px;
            padding: 15px;
            background: #444;
            border-radius: 10px;
        ...
        .chart-box {
            height: 300px; /* Atur tinggi untuk grafik */
        }
        canvas#rpmChart {
...
<body>
    ...
    <div class="container">
        ...
        <!-- Line Chart RPM -->
        <div class="chart-box">
            <h3>📊 Grafik RPM Aktual vs Target (30 Detik)</h3>
    ...
    <script>
        let gaugeRPM;
        let rpmChart; // Global Chart variable
        let isTyping = false; // Flag untuk mencegah konflik input
        const MAX_DATA_POINTS = 60; // Max 60 points (30 seconds at 500ms interval)
        
        // Fungsi untuk inisialisasi grafik Chart.js
        const initChart = (maxRpm) => {
            ...
            rpmChart = new Chart(ctx, {
                    ...
                    datasets: [
                        {
                            label: 'RPM Aktual',
                            data: [],
                            borderColor: 'rgba(255, 235, 59, 1)', // Kuning
                ...
                options: {
                    ...
                    scales: {
                        y: {
                            beginAtZero: true,
                            max: maxRpm || 800, // Batas max Y-axis sama dengan gauge
                            title: { display: true, text: 'RPM', color: 'white' },
                            grid: { color: 'rgba(255, 255, 255, 0.1)' },
                            ticks: { color: 'white' }
                        },
                        x: {
                            title: { display: true, text: 'Waktu (Menit:Detik)', color: 'white' },
                            grid: { display: false },
        ...
        const fetchPIDData = () => { 
            ...
            .then(data => {
                ...
                if (rpmChart) {
                    ...
                    // Tambahkan data baru
                    rpmChart.data.labels.push(timeString);
                    rpmChart.data.datasets[0].data.push(currentRPM);
                    // RPM Target harus selalu sesuai dengan nilai target saat ini
                    rpmChart.data.datasets[1].data.push(targetRPM); 

                    // Batasi jumlah titik data
                    if (rpmChart.data.labels.length > MAX_DATA_POINTS) {
                        rpmChart.data.labels.shift();
                        rpmChart.data.datasets[0].data.shift();
                        rpmChart.data.datasets[1].data.shift();
                    }

                    rpmChart.update('none'); // Update tanpa animasi
                }


                // --- 3. Isi Formulir dengan Nilai Saat Ini (Hanya jika pengguna TIDAK sedang mengetik) ---
                if (!isTyping) {
        ...
        $(document).ready(function() {
            setTimeout(() => {
                ...
                if (gauges.length > 0) {
                    gaugeRPM = gauges[0];
                    // Inisialisasi Chart dengan batas maksimum dari Gauge
                    initChart(800); 
                } else {
                    console.error("Gauge RPM tidak terinisialisasi.");
                    initChart(800); // Default max RPM
                }

                // --- Logika Deteksi Fokus Input ---
